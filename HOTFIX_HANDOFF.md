...
- 긴급 수정 후 live `sw.js`의 `delivery-master-install-v19` 또는 새 배포 버전 확인
...
- v19에는 구역 순서 수량 계산, 도우미 배송 무료/유료, 백업설정 선택형 반복 기록 정정, 지난 날짜 기록 정정, 개발앱 백업 복구가 이미 반영되어 있다.
- 목표는 v19 이후 새 필드 버그만 이 기준 위에서 핫픽스하는 것이다.
...
- v19 기준 커밋/푸시 완료 여부는 `git log -1 --oneline`과 live `sw.js`로 확인한다.
- 이전 원본 `src`/설정/문서 이관 완료
- `current-source/`는 `.gitignore`로 제외된 참고 스냅샷
- live `sw.js`의 캐시명 `delivery-master-install-v19` 확인 필요
