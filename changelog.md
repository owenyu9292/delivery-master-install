# v39 - 2026-09-09 최종 설치본 배포 완료

- 소스 태그509519c 및 APK GitHub v39 공개 완료. 비로그인 다운로드 파일과 폰 설치본 SHA256 동일. 이후 테스트 저장/내보내기 JSON 정리 기준 추가.

- 초기 v39에서 발견한 출발/백업 정정/누락 도우미 버튼0px 간격 수정, 구역 선택과 추가·삭제 간격 보완, 통계/구역 선택 터치 높이48px 적용.
- 간격 검수 추가: 3크기 제어912회, 버튼 완전 스크롤/히트378회, 입력168회. 실패 재현 후 통과 확인.
- 최종 APK 폰 덮어 설치 및 SQLite 해시 보존 확인. 이번 APK를 변경 없이 GitHub에 공개한다. VERIFICATION_v39.md 참조.

# v39 - 2026-09-09 기본 재검수 (이전 준비 이력)

- 로그/시간/도우미/수량의 기호 제거와 자릿수 자동절단을 없애 잘못된 입력 의미가 바뀌지 않게 함. 해당 저장만 막고 재수정 가능.
- 저장 성공 알림을 실제 저장 이후로 이동. 동별 초기화도 저장 성공 후 해당 초안만 삭제해 실패 시 화면 값 보존.
- 신규 기본9묶음/입력168회/공통 숫자106조건을 기본 검수에 포함. VERIFICATION_basics_2026-09-09.md.
- 폰 연결 후 설치할 v39 준비. 공개 배포/커밋/푸시/설치 전.

# 미배포 - 2026-09-09 공통 키보드 여백

- 네이티브 루트와 Capacitor SystemBars의 중복 IME 여백 처리 제거. 처리한 inset은 자식에 0으로 전달하고 Android CSS 추가 여백도 방지.
- 수량/시/분/이름의 키보드 가용 높이 3종 및 닫기 복원 16조건을 기본 브라우저 검수에 추가.
- 전체 자동/브라우저 검사 및 APK 대조 완료. 삼성 키보드 실기기 확인과 배포는 미완료. VERIFICATION_keyboard_2026-09-09.md.

# 미배포 - 2026-09-09 리포트 화면

- 리포트/날짜 조회를 파생 수치 기반 읽기 화면으로 재설계. 작은 내부 스크롤 제거, 탁색 블루/시스템 폰트, 상하단 복사 유지.
- 미주 A/restTotal 등 과거 상세 필드 형식의 총량 정정 시 B가 남는 결함 보완. 비미주 구역 수식 불변.
- 전체 자동 검사 및38+22 브라우저 묶음 통과, 검사용 APK 대조 완료. 새 배포 없음. VERIFICATION_report_2026-09-09.md.

# 미배포 - 2026-09-09 스크롤·뒤로가기

- 실제 하단 메뉴 높이 및 키보드 가용 높이와 스크롤/편집창 연동.
- Android 뒤로가기 즉시 종료 대신 입력/편집/탭 순 복귀. 저장 중 종료 및 Escape/배경 닫기 방지, 연속 누름 보호.
- 기본 조작 자동검사를 전체 검수 경로에 추가. 업무34+저장22묶음 및60개 끝 스크롤 조합 통과. 검사용 APK 빌드 완료, 아직 공개 반영 전.
- 상세/실기기 미확인 경계: VERIFICATION_navigation_2026-09-09.md.

# v38 - 2026-09-09 통계 확장 및 전체 재검수

- 비율 우선 화면, 이전 같은 경과 기간 비교, 6기간 추이, 시간 구성, 물량/요일/구역/도우미/A·B/시각/스캔/자료 품질 분석.
- 통계는 읽기 전용. 미마감·복사본·손상 자료 구분, 예상 수량 미기록의 스캔 차이 과장 방지, 이상 시간/수량 효율 제외와 확인 목록.
- 전체 자동 검사와26+22개 숨김 브라우저 묶음, 2684시간·수량 조합, 통계15묶음+600변형, 401일 자료 불변 확인.
- 4e6c352 소스/APK 커밋·푸시·v38 공개 및 비로그인 다운로드 SHA256 일치 확인. versionCode11/0.2.37, 기존 서명 유지. PWA 불변. VERIFICATION_v38.md 참조.

# v37 - 2026-09-09 시간 연동 및 자동 정리

- timeLinks 공통 도메인: 정리완료/배송시작, 동일 도착/구역 시작, 연속 구역 경계, 최종 완료/업무 종료 양방향 연동. 실제 다른 간격 유지.
- autoCleanup: 힐스 정리40분 누락 시30분을 기록하고 배송 입력 단계로 전환. 복귀/재실행에서도 원래 시작+30분, 반복 호출 중복 없음.
- 수정 후보 연동 후 검증, 자동값 직접 정정 우선, 자정 이후 시각의 날짜 유지, 반복 수정 UUID 충돌 방지.
- 저장 성공 전 원본/화면 유지, 수동 동작과 자동 보정 직렬화. 보정 표시와 바로 수정 연필.
- 도우미 전환 때 구역 원본/동행 기록 보관, 복원 시 시간·ID·상세 유지. 삭제된 구역의 도우미 총수량 누락 수정. 과거 원본이 없는 복원은 임의5분 대신 효율 미확정.
- 가혹 검수: 23개 업무·시간·도우미·통계/22개 저장·정정 브라우저 묶음 및2684교차조합/도우미11묶음. 구형 검사 대신 현재 UI의 격리 검사를 기본 진입점으로 연결. APK v37/versionCode10, 기존 서명 유지. a059908 커밋·푸시·태그 및 공개 APK 다운로드 해시 확인 완료. VERIFICATION_v37.md 참조.

# v36 - 2026-09-09 실제 앱 변경

- 탁색 블루 업무 화면, 하단 아이콘 탭, 큰 누적 수량/자동 차감 미리보기, 미주1/2/3동 기록 및 나머지 계산.
- 구역 추가 경로 통합. 내 구역/대체 선택, 반복 추가/방문, 예정 순서/다음 구역 변경, 예정 삭제 및 마감 확인.
- 종류와 방문ID 분리. 진행 중 종류 변경 후 시간/수량 초안/A기록 보존. 구역명이 아니라 실제 순서로 누적 차감.
- 정리 완료 우선, 진행 중 보정 접힘, 미방문 예정 구역 보고서 제외.
- 입력 오류/저장 실패 시 원본과 편집 상태 보존, 이중 누름 회귀 검수.
- Android 시스템 sans-serif와 글자 배율, 키보드 인셋 적용. 유료 삼성 글꼴 실기기 표시 여부는 미확인.
- 소스/APK 커밋 e59c2d2 및 v36 태그, GitHub Release 게시, 비로그인 다운로드 해시 일치 확인 완료. 검수/배포 증거: VERIFICATION_v36.md. 기존 PWA 원본은 변경하지 않음.

# 2026-09-09 프로젝트 강제 규칙 해제 이력
...
- 승인: 남은 결함 수정 및 하루 자료 손실을 전제로 한 가혹 검수. 기록자 main, 저장/계산은 Luna 분담 후 코기 통합 검수.
- 저장소: 깊은 백업 검증, 원자적 다중 날짜 가져오기, 동기 put 실패 rollback, 손상 날짜 격리/원문 보관/선택 복구. 네이티브 SQLite transaction 중복 시작 방지.
- 화면: 저장 실패 뒤 원본 재조회·편집 입력 유지, 미저장 초안 복원, 자정 선택, 완료 로그 시간 수정, 진행 후 남은 구역 순서 변경.
- 계산: 무료 도우미 효율 분자, 미주 총량 정정 B 동기화, 누락 정리 완료와 자동 배송시작 연결, 미확정 시간의 효율 제외, 통계 가중 효율 및 실제 대체배송 제외 정규 효율.
- UI: 로그 같은 분 업무 순서, 연필48px, 주/월 이동 가로 배치, 로그/통계 장황한 안내 제거.
- 검수: npm run check 통과, 실제 SQLite 파일 재개방/rollback6종, 최종 숨김 브라우저22개 묶음, 기존 smoke/대체배송10개 경로 통과. VERIFICATION_2026-09-05.md 참조.
- Git/APK: 기준35af4fc, codex/hils-handling-minutes에서 미커밋. PWA·dist·assets·android·downloads·버전 변경 없음. 설치·배포 미실행.

# 미배포 - 2026-09-05 힐스 별도 작업 시간

- 기록자: 일반 작업창 / 채널: main. 인스톨 앱만 수정. PWA 원본과 설치 APK는 그대로다.
- src/domain/handlingTime.ts: 구역별 반품·선집화·상차 incident 생성/갱신. 소요분 0~999 정수, 0은 취소, 반복 적용은 합산하지 않고 갱신. 기존 정리와 별개다.
- src/app/main.ts: 힐스 주행동 아래 소요분/기록/취소, 기본30 미적용 제안, 기록 후 표시, 배송 수량 초안 보존. 작업 로그 연필 재수정, 완료 로그에서 누락 시간 추가.
- public/styles.css: 소요분과 명령을 한 줄에 배치하고 터치 높이48px 확보.
- scripts/run-handling-tests.ts: 루나 작성 후 코기 검토. 차감·수량/전체경과 불변·반복/취소·구역ID·오입력·JSON 왕복 검사.
- scripts/browser-handling.mjs: 새 임시 Chrome 프로필, 독립 포트, 임시 빌드로 실제 키/마우스 입력 검사. npm run check:handling으로 재실행 가능. HANDLING_FULL_REGRESSION=1이면 기존 브라우저 검사도 실행한다.
- package.json: 새 도메인 검사를 npm run check에 통합했다.
- 통과: 정리20+별도30=배송70, 기본값 미자동적용, 반복탭, 45분수정/취소0/재적용30, 빈값/음수/소수/문자/1000/Infinity 거부 및 기존값 유지, 새로고침, 완료 후60/0/45 재수정, 누락35 추가 후40 재수정, 다른 힐스ID, 411x762 DPR2.63 및360폭.
- 기존 회귀: 61/61 도메인, 플랫폼, SQLite, runtime, browser-smoke, 대체배송10개 조합 통과. 새 테스트도 통과.
- 미실행: APK 패키징/실기기 설치/배포/커밋/푸시. 버전 번호와 배포 파일은 올리지 않았다.

# v34 - 2026-08-23 로그 현장 정정 보강

- 로그 연필 수정 경로를 일반 입력/완료 수정과 분리했다.
- 시간축이 뒤집힌 로그도 입력값을 우선 저장하고 경고를 adjustments에 기록한다.
- 정리 시작만 남은 구역에 정리 완료 누락 로그 항목과 직접 추가 편집기를 제공한다.
- 로그 완료 수량은 0개와 5자리까지 허용하며 빈 입력만 차단한다.
- 저장 전 log-inline-before 스냅샷을 유지한다.
- PWA/season2는 수정하지 않았다.
- 임시 Chrome 411x762 / DPR 2.63과 CDP 현장 정정 시나리오는 통과했다. Fold7 실기기 검수와 APK 배포는 아직 완료로 처리하지 않았다.
- Android versionCode 7 / versionName 0.2.33으로 v34 debug APK를 빌드했고, 배포 파일 해시를 기록했다.

# v33 - 2026-07-15 추가구역 오입력 안전 취소

- 대체배송·추가구역을 잘못 추가한 뒤 작업 기록이 없으면 즉시 취소하고 이전 단계로 복귀할 수 있다.
- 취소 전 내부 날짜 스냅샷을 저장하며, 구역과 시작 로그를 제거한 뒤 순서를 정규화한다.
- 배송 시작, 정리, 이벤트, 도우미 등 후속 기록이 있으면 단순 취소를 막아 기록 손실을 방지한다.
- 숨김 Chrome 새 프로필에서 정상 흐름과 변칙 10개 경로를 411x762 / DPR 2.63으로 검증했다.
- APK: DeliveryMaster-v33-debug.apk / SHA256 2A7AD1533E204D8D37FE26A965F1BA6F36FD37ABC8E6F5D3FEEA3E6FD0DF858B.
...
- USB/ADB는 일반 배포 경로가 아니라 실기기 검수·긴급 진단 용도로 분리했다.
- 루나의 읽기 전용 문서 충돌 점검 후 코기가 README, instruction, 설치 구조 문서까지 현재 규칙으로 통합했다.


## 2026-07-13 v31 시간축 전면 검증

- 시간 수정 전 공통 시간축 검사를 추가했다.
- 바로 배송 시작은 구역 시작 후 2분을 넘겨 수정하면 저장하지 않는다.
- 정리 전 배송 시작, 구역 순서 중첩, 구역 시작 이전 자동 보정도 차단 또는 보정한다.
- Android versionCode `4`, 앱 `0.2.30-android-install`, cache `v31`.
- GitHub 다운로드용 `DeliveryMaster-v31-debug.apk`를 준비했다.
## 2026-07-13 v30 자동 스냅샷 내부 저장 보정

- 시간/완료 기록 수정 전 자동 안전 스냅샷은 Android 공유창을 열지 않고 앱 내부에 저장하도록 변경.
- 사용자 명시 행동인 `백업 내보내기`만 JSON 공유/저장 선택 창을 연다.

## 2026-07-13 v29 Android 안전영역 및 배송 시작 누락 보정

- Android 설치판의 시스템 상태표시줄/내비게이션 바 겹침을 피하도록 safe-area inset 적용.
- 모든 구역의 배송 시작 누락 자동 보정 추가: 정리 완료 우선, 이전 구역 종료+5분, 첫 구역은 구역 시작/도착 시각 순.
- 완료 기록 수정과 로그 수정에서도 배송 시작을 수정/복구 가능.
- 앱 버전 `0.2.28-android-install`, cache `v29`, Android versionCode `2`.
- PWA v1/season2 수정 없음.

# 2026-07-12 Android 설치판 이관 및 현장 진입 보정

- 기록자: 일반 작업창 / 채널: main
- Android SQLite 설치판에 개발앱 전체 백업 102일 이관을 실기기에서 성공 확인했다.
- 가져오기 전후 자동 외부 공유가 `Share canceled`로 이관 전체를 막던 문제를 수정했다. 안전 스냅샷은 Android 앱 내부 저장으로 보관하고, 외부 공유는 사용자가 직접 내보낼 때만 연다.
- 대량 이관 확인창은 날짜 102개 나열 대신 기간 요약으로 표시한다.
- 이관 후 업무 탭이 첫 과거 날짜를 선택해 오늘 업무를 시작하지 못하던 문제를 수정했다. 이관 후 항상 오늘의 빈 업무 기록으로 복귀한다.
- 상단 `PHONEINSTALL ALPHA`를 제거하고 `배송마스터 v28`로 정리했다.
- Android 뒤로가기 버튼은 앱 종료로 동작한다.
- Android 적응형 런처 아이콘을 택배 트럭으로 교체했다.
- 검수: `npm run check` 50/50, 플랫폼/SQLite/런타임 검사 통과. Android debug APK 빌드 및 폴드7 데이터 유지 재설치 성공.
- 운영 기준: 현장앱은 PWA 긴급 대응본, 인스톨판은 정식 반영·검수·커밋 대상이다. 구버전 PWA v1/season2는 보관용이다.
...
- 2026-07-05 v24 `alt-zone-priority`
  ...
  - 진행 중 대체배송은 현재 작업 구역으로 우선 표시한다.
  - 새 대체배송은 다음 미시작 구역 앞에 확실히 삽입한다.
...
- 2026-06-17 v21 `risk-quantity-recovery`
  ...
  - 위험 수량은 저장을 멈추고 `입력 다시 하기`, `누적 총합으로 계산`, `이 구역 실제 수량 저장`, `예외 저장` 복구 패널을 먼저 보여준다.
  - 예외 저장은 `quantityOverride`와 사유를 timeline payload에 남긴다.
  - 기록 정정의 날짜+시간 입력칸을 시간 전용 입력칸으로 바꿔 폰 화면 잘림 위험을 줄였다.
  - 브라우저 스모크에 위험 수량 패널, confirm 미사용, 재입력 복구, 힐스 선진행, 미주 후진행, 대체배송/추가구역 누적 차감 검수를 추가했다.
  - 검수: `npm run check` 50/50 통과, `npm run check:codex` 통과, 411x762 / DPR 2.63 브라우저 스모크 통과.

- 2026-06-16 v20 `zone-helper-dedup`
  - 기록자: 일반 작업창 / 채널: main
  - 구역 위치에서 기록한 무료/유료 도우미를 구역 내부 동행/기여로 처리하게 했다.
  - 구역 동행 도우미 수량이 총 배송 수량과 효율 분자에 중복 합산되지 않게 했다.
  - 구역 동행 도우미는 수량을 비워도 저장할 수 있게 했다.
  - 리포트에 구역 동행 무료/유료 수량은 `총량 중복 제외`로 표시한다.
  - 도메인 테스트에 유료 도우미 50개 중복 방지와 수량 미기록 동행 케이스를 추가했다.
  - 브라우저 스모크에 힐스 완료 후 구역 위치 유료 도우미 50개/빈칸 동행을 기록해도 총 배송 수량이 늘지 않는 검수를 추가했다.
